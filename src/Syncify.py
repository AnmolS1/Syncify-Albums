...
class DataHandler:
	...
	def spotify_extractor(self, link):
		...
		if "album" in link:
			...
			fields = "items(track(name,artists(name),album(name)),added_at)"
		...
		try:
			...
			with concurrent.futures.ThreadPoolExecutor(max_workers=self.thread_limit) as executor:
				...
				for song in playlist_tracks:
					...
					song_album = song['Folder']
					playlist_folder_full_path = os.path.join(self.download_folder, song_album)
					...
					if cleaned_full_file_name not in directory_list:
						...
						if song.get("VideoID"):
							...
							futures.append((future, cleaned_full_file_name, song_album))
	...
	def download_queue(self, song_list, playlist):
		# this file is where we'll put each individual song pathlink
	...
	def string_cleaner(self, input_string) -> str:
		raw_string = re.sub(r'[\/:*?"<>|]', " ", input_string)
		temp_string = re.sub(r"\s+", " ", raw_string)
		cleaned_string = temp_string.strip()
		return cleaned_string
	
	def list_cleaner(self, input_list) -> list[str]:
		return[self.string_cleaner(s) for s in input_list]
